    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          ...
          description: errorData.message || "Impossibile salvare i supermercati selezionati",
        })
      }
    } catch (error) {
      toast({
    ...
    <div className="min-h-screen bg-gradient-to-br from-neutral-50 to-neutral-100 px-4 py-6">
      <div className="max-w-md mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <Badge variant="secondary">{selectedSupermarkets.length} selezionati</Badge>
        </div>

        {/* Title */}
        <div className="text-center mb-8">
          <h1 className="text-2xl font-bold text-neutral-900 mb-2">Scegli i tuoi supermercati</h1>
          <p className="text-neutral-600">Seleziona dove fai solitamente la spesa</p>
        </div>
          ...
          {SUPERMARKETS.map((supermarket) => (
            ...
            >
              <CardContent className="p-4">
                ...
                >
                  <span className="text-white font-bold text-sm text-center px-2">{supermarket.name}</span>
                  {selectedSupermarkets.includes(supermarket.id) && (
                ...
                <p className="text-sm text-neutral-700 text-center font-medium">{supermarket.name}</p>
              </CardContent>
            </Card>
          ))}
        </div>
